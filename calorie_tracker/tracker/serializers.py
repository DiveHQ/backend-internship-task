...
from .helpers import retrieve_calories
...
class EntrySerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        text = attrs.get("text")
        calories = attrs.get("calories", None) or retrieve_calories(query=text)
    ...
    def _get_total_calories_for_today(self, user: User):
        ...
        user_entries_for_today = Entry.objects.filter(user=user, date=date.today())
        total_calories_for_today = (
            user_entries_for_today.aggregate(Sum("calories"))["calories__sum"] or 0
        )
